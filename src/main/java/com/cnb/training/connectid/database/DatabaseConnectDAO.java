...
public class DatabaseConnectDAO {
    ...
    protected Connection getConnectionAdmin() throws Exception {
        DataSource ds = null;
        ...
        try {
            ...
            ds = (DataSource) envContext.lookup("jdbc/CNBadmin");
            ...
            throw new Exception("SQL exception  getting connection: " + e.toString());
        }

        if (conn == null) {
            throw new Exception("No  Datasource found for admin");
        }
        return conn;
    }


    protected Connection getConnectionResources() throws Exception {
        DataSource ds = null;
        Connection conn = null;
        try {
            InitialContext initialContext = new InitialContext();
            ...
            ds = (DataSource) envContext.lookup("jdbc/ChemNetBaseResources");

            conn = ds.getConnection();
            conn.setAutoCommit(false);
            //  DatabaseMetaData mtdt = conn.getMetaData();

            // String useris = mtdt.getUserName();
